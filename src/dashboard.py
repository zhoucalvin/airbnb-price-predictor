...
# App layout
# ─────────────────────────────────────────────────────────────────────────────
app = Dash(__name__, title="Airbnb Price Predictor", suppress_callback_exceptions=True)

TAB_STYLE        = {"padding": "6px 16px", "fontWeight": "500"}
TAB_SELECTED     = {"padding": "6px 16px", "fontWeight": "700", "borderTop": "3px solid #FF5A5F"}
CARD_STYLE       = {"background": "#fff", "borderRadius": "8px", "padding": "20px",
                    "boxShadow": "0 1px 4px rgba(0,0,0,0.1)", "marginBottom": "16px"}
LABEL_STYLE      = {"fontWeight": "600", "fontSize": "13px", "marginBottom": "4px", "display": "block"}

app.layout = html.Div(style={"fontFamily": "Inter, sans-serif", "background": "#f5f5f5", "minHeight": "100vh"}, children=[

    # Header
    html.Div(style={"background": "#FF5A5F", "color": "white", "padding": "20px 32px"}, children=[
        html.H1("Airbnb Price Predictor", style={"margin": 0, "fontSize": "24px"}),
        html.P("NYC · Los Angeles · Chicago | CIS 2450 Final Project",
               style={"margin": "4px 0 0", "opacity": 0.85, "fontSize": "14px"}),
    ]),

    # Tabs
    dcc.Tabs(id="tabs", value="tab-predict", style={"background": "#fff"}, children=[
    ...
    return html.Div([
        html.Div(style={"display": "grid", "gridTemplateColumns": "340px 1fr", "gap": "24px"}, children=[
            ...
            html.Div(style=CARD_STYLE, children=[
                html.H3("Listing Details", style={"marginTop": 0, "fontSize": "16px"}),
            ...
            # Results panel
            html.Div([
                html.Div(id="prediction-output", style=CARD_STYLE),
                html.Div(id="nearby-map-container", style=CARD_STYLE),
            ]),
        ]),
    ])
...
@app.callback(
    ...
    Output("nearby-map-container", "children"),
...
def predict_price(n_clicks, city, neighbourhood, room_type, bedrooms, bathrooms,
    ...
    if xgb_pred is None:
        return html.P("Prediction failed — check inputs."), html.P("")
    ...
    # Prediction card
    pred_card = html.Div([
        html.H3("Predicted Nightly Price", style={"marginTop": 0}),
        html.Div(style={"display": "flex", "gap": "32px", "alignItems": "center", "flexWrap": "wrap"}, children=[
            html.Div([
                html.Div(f"${xgb_pred:,.0f}", style={"fontSize": "48px", "fontWeight": "800", "color": "#FF5A5F"}),
                html.Div(f"Range: ${low:,.0f} - ${high:,.0f}",
                         style={"fontSize": "14px", "color": "#666", "marginTop": "4px"}),
                html.Div("Based on XGBoost model (best performer)",
                         style={"fontSize": "12px", "color": "#999", "marginTop": "2px"}),
            ]),
            html.Div([
                html.Table(style={"fontSize": "13px", "borderCollapse": "collapse"}, children=[
                    html.Tbody([
                        html.Tr([html.Td("Ridge", style={"paddingRight": "16px", "color": "#999"}),
                                 html.Td(f"${preds['Ridge']:,.0f}" if preds.get("Ridge") else "—",
                                         style={"fontWeight": "600"})]),
                        html.Tr([html.Td("Random Forest", style={"paddingRight": "16px", "color": "#999"}),
                                 html.Td(f"${preds['Random Forest']:,.0f}" if preds.get("Random Forest") else "—",
                                         style={"fontWeight": "600"})]),
                        html.Tr([html.Td("XGBoost", style={"paddingRight": "16px", "color": "#999"}),
                                 html.Td(f"${preds['XGBoost']:,.0f}" if preds.get("XGBoost") else "—",
                                         style={"fontWeight": "600", "color": "#FF5A5F"})]),
                    ])
                ])
            ]),
        ]),
    ...
    map_fig = px.scatter_map(
        nearby, lat="latitude", lon="longitude", color="price_usd",
        color_continuous_scale="Viridis",
        hover_data=["price_usd", "room_type", "neighbourhood_cleansed"],
        zoom=11, center={"lat": lat_c, "lon": lon_c},
        map_style="carto-positron",
        title=f"Comparable Listings in {CITY_LABELS[city]} (${price_low:.0f}-${price_high:.0f}/night)",
        opacity=0.75, template=PLOTLY_TPL, height=400,
    )
    map_fig.update_layout(coloraxis_colorbar_title="Price (USD)", margin={"t": 40, "b": 0})

    map_card = html.Div([
        html.H3("Comparable Nearby Listings", style={"marginTop": 0}),
        dcc.Graph(figure=map_fig, config={"displayModeBar": False}),
    ])

    return pred_card, map_card
...
def render_eda():
    ...
    price_fig = px.histogram(
        df.sample(10000, random_state=42), x="price_usd", nbins=60,
        color="city", color_discrete_map=CITY_COLORS,
        title="Price Distribution by City (sample of 10k)",
    ...
    violin_fig = px.violin(
        df.sample(10000, random_state=42), x="city", y="log_price",
        color="city", color_discrete_map=CITY_COLORS,
    ...
    # Median price by neighbourhood (top 15 per city)
    nbhd_stats = (
    ...
    top_nbhd = (
        nbhd_stats.sort_values("median_price", ascending=False)
        .groupby("city").head(15)
        .sort_values(["city", "median_price"])
    )
    nbhd_fig = px.bar(
        top_nbhd, x="median_price", y="neighbourhood_cleansed",
        color="city", color_discrete_map=CITY_COLORS,
        facet_col="city", orientation="h",
        title="Top 15 Neighbourhoods by Median Price (per city)",
        labels={"median_price": "Median Price (USD)", "neighbourhood_cleansed": ""},
        template=PLOTLY_TPL, height=500,
    )
    nbhd_fig.update_yaxes(matches=None)

    # Walk score vs log price scatter
    ...
    return html.Div([
        html.Div(style={"display": "grid", "gridTemplateColumns": "1fr 1fr", "gap": "16px"}, children=[
            html.Div(style=CARD_STYLE, children=[dcc.Graph(figure=price_fig)]),
            html.Div(style=CARD_STYLE, children=[dcc.Graph(figure=violin_fig)]),
            html.Div(style=CARD_STYLE, children=[dcc.Graph(figure=ws_fig)]),
            html.Div(style=CARD_STYLE, children=[dcc.Graph(figure=nbhd_fig)]),
        ]),
    ])
...
def render_models():
    ...
    rmse_fig.update_traces(textposition="outside")
    ...
    r2_fig.update_traces(textposition="outside")
    r2_fig.update_layout(showlegend=False)
        ...
        children=[
            html.Thead(html.Tr([
                html.Th(col, style={"textAlign": "left", "padding": "8px 12px",
                                    "borderBottom": "2px solid #eee", "fontWeight": "700"})
                for col in metrics_df.columns
            ])),
            html.Tbody([
                html.Tr([
                    html.Td(metrics_df.iloc[i][col],
                            style={"padding": "8px 12px", "borderBottom": "1px solid #f0f0f0",
                                   "color": "#FF5A5F" if i == 2 else "inherit"})
                    for col in metrics_df.columns
    ...
    # SHAP bar chart (pre-computed)
    shap_section = html.P("Run xgboost_model.ipynb to generate SHAP outputs.",
                          style={"color": "#999", "fontStyle": "italic"})
    shap_img_path = ROOT / "outputs" / "eda" / "shap_summary.png"
    if shap_img_path.exists():
        import base64
        img_b64 = base64.b64encode(shap_img_path.read_bytes()).decode()
        shap_section = html.Img(
    ...
    return html.Div([
        html.Div(style=CARD_STYLE, children=[
            html.H3("Model Performance Summary", style={"marginTop": 0}),
            table,
        ]),
        html.Div(style={"display": "grid", "gridTemplateColumns": "1fr 1fr", "gap": "16px"}, children=[
            html.Div(style=CARD_STYLE, children=[dcc.Graph(figure=rmse_fig)]),
            html.Div(style=CARD_STYLE, children=[dcc.Graph(figure=r2_fig)]),
        ]),
        html.Div(style=CARD_STYLE, children=[
            html.H3("SHAP Feature Importance - XGBoost", style={"marginTop": 0}),
            shap_section,
        ]),
    ])
